...
#define BTPM_CLIENT_ERROR_MESSAGE_ERROR_HANDLER_NOT_REGISTERED 0x00000001
	...
	/* The following structure represents the Message definition for a   */
	/* generic response message (containing only status word)            */
	/* (Response).                                                       */
typedef struct _tagGeneric_Response_t
{
	BTPM_Message_Header_t MessageHeader;
	int                   Status;
} BTPM_Generic_Response_t;


/* The following is a event id for a generic User Event which will be    */
/* parsed on the client app (not in IPC layer)                           */
#define BTPM_MESSAGE_FUNCTION_USER_EVENT                     0xFFFE0001


typedef struct _UserEventData_t
{
	unsigned int 	 		EventType;        /* Defined Per Module */
	unsigned int     		EventLength;
	unsigned int     		EventCallbackID;  /* Defined Per Module */
	Byte_t			 		VariableData[1];  /* Defined Per Module */
} BTPM_UserEventData_t;


/* The following are registration and un-registration function id for
 * PM Client                                                             */
#define BTPM_MESSAGE_FUNCTION_OPEN_CLIENT                    0xFFFF0001
#define BTPM_MESSAGE_FUNCTION_CLOSE_CLIENT                   0xFFFF0002

/* Request Structure for BTPM_MESSAGE_FUNCTION_UN_REGISTER_CLIENT */
typedef struct _BTPM_RegistrationReq_t
{
	unsigned int Role;
} BTPM_RegistrationReq_t;

/* Request Structure for BTPM_MESSAGE_FUNCTION_UN_REGISTER_CLIENT */
typedef struct _BTPM_RegistrationRsp_t
{
	BTPM_Message_Header_t MessageHeader;
	int                   Status;
	void                  *hSocket;
} BTPM_RegistrationRsp_t;
