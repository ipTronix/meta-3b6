...
#define HFRM_MESSAGE_FUNCTION_SEND_AVAILABLE_CODECS            0x00001410
#define HFRM_MESSAGE_FUNCTION_SEND_SELECT_CODEC                0x00001411
...
#define HFRM_MESSAGE_FUNCTION_CODEC_SELECT_IND                 0x00012014
#define HFRM_MESSAGE_FUNCTION_CODEC_SELECT_CFM                 0x00012015
#define HFRM_MESSAGE_FUNCTION_CODEC_CONNECTION_SETUP_IND       0x00012016
...
#define HFRM_SEND_ARBITRARY_RESPONSE_RESPONSE_SIZE             (sizeof(HFRM_Send_Arbitrary_Response_Response_t))
   ...
   /* The following structure represents the message definition for a   */
   /* Hands Free Manager message to send an available codecs to the     */
   /* remote Hands Free device (Request).                               */
   /* * NOTE * This is the message format for the                       */
   /*                                                                   */
   /*             HFRM_MESSAGE_FUNCTION_SEND_AVAILABLE_CODECS           */
   /*                                                                   */
   /*          Message Function ID.                                     */
typedef struct _tagHFRM_Send_Available_Codecs_Request_t
{
   BTPM_Message_Header_t MessageHeader;
   unsigned int          ControlEventsHandlerID;
   BD_ADDR_t             RemoteDeviceAddress;
   unsigned int          NumberAvailableCodecs;
   unsigned char         CodecIDList[1];
} HFRM_Send_Available_Codecs_Request_t;

   /* The following MACRO is provided to allow the programmer a very    */
   /* simple means of quickly determining the total number of bytes that*/
   /* will be required to hold an entire send available codecs request  */
   /* message given the length (in characters) of the arbitrary         */
   /* response.  This MACRO accepts as it's input the total number of   */
   /* characters (including the NULL terminator) that are present       */
   /* starting from the ArbitraryResponse member of the                 */
   /* HFRM_Send_Available_Codecs_Request_t structure and returns the    */
   /* total number of bytes required to hold the entire message.        */
#define HFRM_SEND_AVAILABLE_CODECS_REQUEST_SIZE(_x)          (STRUCTURE_OFFSET(HFRM_Send_Available_Codecs_Request_t, CodecIDList) + (unsigned int)((sizeof(char)*((unsigned int)(_x)))))

   /* The following structure represents the message definition for a   */
   /* Hands Free Manager message to send an available codecs response to*/
   /* the remote Hands Free device (Response).                          */
   /* * NOTE * This is the message format for the                       */
   /*                                                                   */
   /*             HFRM_MESSAGE_FUNCTION_SEND_AVAILABLE_CODECS           */
   /*                                                                   */
   /*          Message Function ID.                                     */
typedef struct _tagHFRM_Send_Available_Codecs_Response_t
{
   BTPM_Message_Header_t MessageHeader;
   int                   Status;
} HFRM_Send_Available_Codecs_Response_t;

#define HFRM_SEND_AVAILABLE_CODECS_RESPONSE_SIZE             (sizeof(HFRM_Send_Available_Codecs_Response_t))

   /* The following structure represents the message definition for a   */
   /* Hands Free Manager message to send a select codec request to the  */
   /* remote Hands Free device (Request).                               */
   /* * NOTE * This is the message format for the                       */
   /*                                                                   */
...
typedef struct _tagHFRM_Send_Select_Codec_Request_t
{
   ...
   /* The following MACRO is provided to allow the programmer a very    */
   /* simple means of quickly determining the total number of bytes that*/
   /* will be required to hold an entire send select codec request      */
   /* message given the length (in characters) of the arbitrary         */
   /* response.  This MACRO accepts as it's input the total number of   */
   /* characters (including the NULL terminator) that are present       */
   /* starting from the ArbitraryResponse member of the                 */
   /* HFRM_Send_Select_Codec_Request_t structure and returns the total  */
   /* number of bytes required to hold the entire message.              */
...
#define HFRM_ARBITRARY_COMMAND_INDICATION_MESSAGE_SIZE(_x)     (STRUCTURE_OFFSET(HFRM_Arbitrary_Command_Indication_Message_t, ArbitraryCommand) + (unsigned int)((sizeof(char)*((unsigned int)(_x)))))
   ...
   /* The following structure represents the message definition for a   */
   /* Hands Free Manager message that informs the local Audio Gateway   */
   /* client of a received available codec list command indication event*/
   /* (asynchronously).                                                 */
   /* * NOTE * This is the message format for the                       */
   /*                                                                   */
...
typedef struct _tagHFRM_Available_Codec_List_Indication_Message_t
{
   ...
   unsigned char         AvailableCodecList[1];
} HFRM_Available_Codec_List_Indication_Message_t;

   /* The following MACRO is provided to allow the programmer a very    */
   /* simple means of quickly determining the total number of bytes that*/
   /* will be required to hold an entire codec list command indication  */
   /* message given the length (in characters) of the arbitrary command.*/
   /* This MACRO accepts as it's input the total number codecs that are */
   /* present starting from the AvailableCodecList member of the        */
   /* HFRM_Available_Codec_List_Indication_Message_t structure and      */
   /* returns the total number of bytes required to hold the entire     */
   /* message.                                                          */
#define HFRM_AVAILABLE_CODEC_LIST_INDICATION_MESSAGE_SIZE(_x)     (STRUCTURE_OFFSET(HFRM_Available_Codec_List_Indication_Message_t, AvailableCodecList) + (unsigned int)((sizeof(unsigned char)*((unsigned int)(_x)))))

   /* The following structure represents the message definition for a   */
   /* Hands Free Manager message that informs the local Audio Gateway   */
   /* client of a received codec select indication event                */
   /* (asynchronously).                                                 */
   /* * NOTE * This is the message format for the                       */
   /*                                                                   */
   /*             HFRM_MESSAGE_FUNCTION_CODEC_SELECT_IND                */
   /*                                                                   */
   /*          Message Function ID.                                     */
...
#define HFRM_CODEC_SELECT_INDICATION_MESSAGE_SIZE     (sizeof(HFRM_Codec_Select_Indication_Message_t))
   ...
   /* * NOTE * This is the message format for the                       */
   /*                                                                   */
   /*             HFRM_MESSAGE_FUNCTION_CODEC_SELECT_CFM                */
   /*                                                                   */
   /*          Message Function ID.                                     */
...
#define HFRM_CODEC_SELECT_CONFIRMATION_MESSAGE_SIZE     (sizeof(HFRM_Codec_Select_Confirmation_Message_t))
   ...
   /* The following structure represents the message definition for a   */
   /* Hands Free Manager message that informs the local Audio Gateway   */
   /* client of a received codec connection setup indication event      */
   /* (asynchronously).                                                 */
   /* * NOTE * This is the message format for the                       */
   /*                                                                   */
   /*             HFRM_MESSAGE_FUNCTION_CODEC_CONNECTION_SETUP_IND      */
   /*                                                                   */
   /*          Message Function ID.                                     */
