...
static int HSP_Init(char *devname, unsigned long flags);
...
static int HSP_Init(char *devname, unsigned long flags)
{
	...
	{
	   ParamList.NumberofParameters = 1;
	   ParamList.Params[0].intParam = 0x1112;
	   ...
	   for(i=1; i<argc; i++)
	   {
		   ...
		   if(argv[i][0] == '-')
		   {
			   ...
			   case 'h':
				   ...
				   printf("        Options: -i : Trigger HSP initialization\n");
				   printf("                 -d : Enable FTS log\n");
				   printf("                 -h : Display this usage message\n");
				   printf("                 device-name :: Set device name (default: TI-BT-HSP)\n");
				   exit(0);
				   break;
			   }
		   }
...
int main(int argc, char *argv[])
{
	...
	{
		ret_val = HSP_Init(localdevname, flags);
	}
