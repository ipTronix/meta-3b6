...
/*   10/21/15  D. Keren       Add audio routing to WiLink and CC256x          */
/******************************************************************************/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#define CMD_LINE_FLAGS_SET_DEBUG  0x00000002           /* function to init  */
                                                       /* the HFP with debug*/
#define HFRE_AG_CODEC_SELECTION_SUPPORTED_BIT (0x0200)
...
static Byte_t AudioDataTone_1KHz_16Bit[] =
{
   ...
   * 16bit channel = 16, with offset = 0, driven at rising edge = 0
	 ...
	 *   Fsync Frequency [Hz]
	 *   PCM Frequency [KHz]
	 *   ClockDirection    0 - BT device Clock Master, 1 - BT device is slave
	 *   FsyncEdge         0 = rising edge, 1 = falling edge
	 *   FSyncPolarity     0 - active high, 1 - active low
	 *   PCMChannelConfig_t
	 *      CH1Out
	 *      CH1In
	 *      CH2Out
...
PCMCodecConfig_t PCMCodecConfig16k =
	 {16000, 1600, 1, 1, 1, {16, 0, 0}, {16, 0, 0}, {16, 20, 0}, {16, 20, 0} };
*/
PCMCodecConfig_t PCMCodecConfig16k =
//   {16000, 512, 1, 1, 1, {16, 0, 0}, {16, 0, 0}, {16, 16, 0}, {16, 16, 0} };
   {16000, 512, 0, 0, 1, {16, 1, 0}, {16, 1, 0}, {16, 17, 0}, {16, 17, 0} };
//   {16000, 512, 0, 1, 1, {16, 16, 0}, {16, 16, 0}, {16, 0, 0}, {16, 0, 0} };

  /* The 8KHz (for CVSD) configuration. Just the Fsync frequency and PCM clock changed  */
/*
PCMCodecConfig_t PCMCodecConfig8k =
	 {8000, 800, 1, 1, 1, {16, 0, 0}, {16, 0, 0}, {16, 20, 0}, {16, 20, 0} };
*/
PCMCodecConfig_t PCMCodecConfig8k =
	 {8000, 256, 0, 1, 1, {16, 0, 0}, {16, 0, 0}, {16, 16, 0}, {16, 16, 0} };
...
static int HFRMSendAvailableCodecs(ParameterList_t *TempParam);
...
static int HFP_Init(char *devname, unsigned long flags);
...
static void UserInterface(void)
{
   ...
   AddCommand("QUERYREMOTEINDICATORSSTATUS", HFRMQueryRemoteIndicatorsStatus);
   ...
   AddCommand("SENDAVAILABLECODECS", HFRMSendAvailableCodecs);
...
/* The following function prints the UI prompt according the compiled device */
static void PrintPrompt(void)
{
#ifdef wl18xx
	printf("Hands Free wl18xx>");
#endif

#ifdef cc256x
	printf("Hands Free cc256x>");
#endif
}

/* Shell commands to activate the GStreamer in 16KHz */
#define SAMPLE_RATE_16KHZ   				(16)
#define ROUTE_GSTREAMER_WL_TO_AM335_SPK_16K "gst-launch-1.0 alsasrc device=plughw:imxwilinkbt ! audio/x-raw,channels=1,rate=16000 ! alsasink device=plughw:sgtl5000 &"
#define ROUTE_GSTREAMER_AM335_MIC_TO_WL_16K	"gst-launch-1.0 alsasrc device=plughw:sgtl5000 ! audio/x-raw,channels=2,rate=16000 ! alsasink device=plughw:imxwilinkbt &"
/* Shell commands to activate the GStreamer in 8KHz */
#define SAMPLE_RATE_8KHZ    				(8)
#define ROUTE_GSTREAMER_WL_TO_AM335_SPK_8K 	"gst-launch-1.0 alsasrc device=plughw:imxwilinkbt ! audio/x-raw,channels=1,rate=8000 ! alsasink device=plughw:sgtl5000 &"
#define ROUTE_GSTREAMER_AM335_MIC_TO_WL_8K 	"gst-launch-1.0 alsasrc device=plughw:sgtl5000 ! audio/x-raw,channels=2,rate=8000 ! alsasink device=plughw:imxwilinkbt &"
 ...
 *
 *  For porting to other boards the audio devices added to the AM335x-EVM- imxwilinkbt and
...
static void RouteGStreamer(int freq)
{
	...
	if(SAMPLE_RATE_8KHZ == freq)
	{
		...
		if(SAMPLE_RATE_8KHZ != gstream_sr) {
			...
			/* Wait 1 second to the process to be killed */
			...
			if(cmd_return_val) {
				...
				/* Wait 1 second to the process to start */
				cmd_return_val = system(ROUTE_GSTREAMER_CMD_SLEEP_1SEC);
...
static int DisplayHelp(ParameterList_t *TempParam)
{
   ...
   printf("*                  45)QueryRemoteIndicatorsStatus                *\r\n");
   ...
   printf("*                  70)EnableBluetoothDebug                       *\r\n");
   printf("*                  71)EnableHDK                                  *\r\n");
   printf("*                  Help, Quit.                                   *\r\n");
   printf("******************************************************************\r\n");
   ...
   if(Initialized)
   {
      ...
      if((TempParam) && (TempParam->NumberofParameters >= 1))
      {
        ...
        if(!HDK_Platform) {
          ...
          printf(" Set CODEC PCM Configuration to 16KHz and Route the GStreamer \r\n");
          ...
          printf(" Set CODEC PCM Configuration to 8KHz and Route the GStreamer \r\n");
          PM_VS_PCM_Codec_Config(&PCMCodecConfig8k);
...
static int HFRMQueryRemoteIndicatorsStatus(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(HFREventCallbackID)
      {
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: QueryRemoteIndicatorsStatus, [BD_ADDR].\r\n");
...
static int HFRMQueryRemoteCallHoldingMultipartyServiceSupport(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      if(HFREventCallbackID)
      {
         ...
         {
            /* One or more of the necessary parameters is/are invalid.  */
            printf("Usage: QueryRemoteCallHoldSupport [BD_ADDR].\r\n");
...
static int HFRMSendArbitraryCommand(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for sending the list of     */
   /* available codecs to the Remote Audio Gateway Device.  This        */
   /* function returns zero on successful execution and a negative value*/
   /* on all errors.                                                    */
static int HFRMSendAvailableCodecs(ParameterList_t *TempParam)
{
   ...
   if(Initialized)
   {
      ...
      {
         /* Make sure the passed in parameters appears to be semi-valid.*/
         if((TempParam) && (TempParam->NumberofParameters >= 2) && (TempParam->NumberofParameters <= (1 + (sizeof(CodecIDList)/sizeof(CodecIDList[0])))))
         {
            /* Convert the parameter to a Bluetooth Device Address.     */
            StrToBD_ADDR(TempParam->Params[0].strParam, &BD_ADDR);

            for(Index = 1; Index < TempParam->NumberofParameters; Index++)
               CodecIDList[Index - 1] = (unsigned char)(TempParam->Params[Index].intParam);

            /* The Port ID appears to be is a semi-valid value.  Now    */
            /* submit the command.                                      */
            Result = HFRM_Send_Available_Codecs(HFREventCallbackID, BD_ADDR, (unsigned int)(TempParam->NumberofParameters - 1), CodecIDList);
            ...
            {
               /* The function was submitted successfully.              */
               printf("HFRM_Send_Available_Codecs: Function Successful.\r\n");
            }
            else
            {
               /* There was an error submitting the function.           */
               printf("HFRM_Send_Available_Codecs: Failure: %d (%s).\r\n", Result, ERR_ConvertErrorCodeToString(Result));
            }
         }
         else
         {
            /* One or more of the parameters are invalid.               */
            printf("Usage: SendAvailableCodecs [BD_ADDR] [Codec1] [CodecN...].\r\n");
...
static void HFRM_Event_Callback(HFRM_Event_Data_t *EventData, void *CallbackParameter)
{
   char                BoardStr[13];
   unsigned int        Index;
   int                 Result;
   ...
   if(EventData != NULL)
   {
      ...
      switch(EventData->EventType)
      {
         ...
         case hetHFRServiceLevelConnectionEstablished:
            ...
            /* No configuration again for WL18xx or HDK platform */
            if(HDK_Platform) break;

#ifdef cc256x
            /* Check if the AG support the CODEC negotiation, then WBS is enable most of the times */
            if(EventData->EventData.ServiceLevelConnectionEstablishedEventData.RemoteSupportedFeatures & HFRE_AG_CODEC_SELECTION_SUPPORTED_BIT)
            {
              Word_t  ConnectionHandle;
             /* Send Vendor Specific command to the CC256x Bluetooth FW. Configure the PCM   *
              *  interface parameters to: 16KHz sample rate, PCM clock rate 1.6MHz, PCM slave *
              *  FSynch- 1 clock with inverted polarity, 16 bit sample with no offset. Route  *
              *  the PCM lines with the GStreamer to 16KHz. Send Vendor Specific command to   *
              *  enable the WBS feature.														*/
             RouteGStreamer(SAMPLE_RATE_16KHZ);
             PM_VS_PCM_Codec_Config(&PCMCodecConfig16k);

             DEVM_QueryConnectionHandle(EventData->EventData.ServiceLevelConnectionEstablishedEventData.RemoteDeviceAddress,
                                        &ConnectionHandle);
             PM_VS_EnableWBS(ConnectionHandle);
             printf("\r\n Set WBS enable for CC256xB with 16Khz sample rate \r\n");
            }
#endif
         ...
         case hetHFRAudioConnected:
            ...
            /* No configuration for HDK platform */
            if(HDK_Platform) break;

#ifdef cc256x
            /* Send again the PCMI configuration to the FW */
            if(gstream_sr == SAMPLE_RATE_16KHZ) {
              printf("\r\n Configure PCM 16KHz \r\n");
              PM_VS_PCM_Codec_Config(&PCMCodecConfig16k);
            } else {
              printf("\r\n Configure PCM 8KHz \r\n");
         ...
         case hetHFRVoiceRecognitionIndication:
            ...
            break;
         case hetHFRCodecSelectIndication:
            BD_ADDRToStr(EventData->EventData.CodecSelectIndicationEventData.RemoteDeviceAddress, BoardStr);

            printf("hetHFRCodecSelectIndication, BD_ADDR: %s, Codec: %d\r\n", BoardStr, EventData->EventData.CodecSelectIndicationEventData.CodecID);

            /* Accept the codec select request.                         */
            Result = HFRM_Send_Select_Codec(HFREventCallbackID, hctHandsFree, EventData->EventData.CodecSelectIndicationEventData.RemoteDeviceAddress, EventData->EventData.CodecSelectIndicationEventData.CodecID);
            ...
            if(HDK_Platform) {
              /* The HDK Platform has on-board CODEC and Audio in/out Jacks.            */
              printf("\r\n No audio configuration needed for HDK platform \r\n");
              break;
            }
...
#ifdef cc256x
            if(HFRE_MSBC_CODEC_ID == EventData->EventData.CodecSelectIndicationEventData.CodecID)
            {
              Word_t  ConnectionHandle;
              /* Send Vendor Specific command to the CC256x Bluetooth FW. Configure the PCM    *
               *  interface parameters to: 16KHz sample rate, PCM clock rate 1.6MHz, PCM slave *
              ...
              DEVM_QueryConnectionHandle(EventData->EventData.CodecSelectIndicationEventData.RemoteDeviceAddress,
                                         &ConnectionHandle);
              PM_VS_EnableWBS(ConnectionHandle);

              printf("\r\n WBS is enabled for CC256xB with PCM 16Khz sample rate \r\n");
            }
            else
            {
              /* Send Vendor Specific command to the CC256x Bluetooth FW. Configure the PCM    *
               *  interface parameters to: 8KHz sample rate, PCM clock rate 0.8MHz, PCM slave  *
               *  FSynch- 1 clock with inverted polarity, 16 bit sample with no offset. Route  *
               *  the PCM lines with the GStreamer to 8KHz.                                    */
              PM_VS_DisableWBS();
              RouteGStreamer(SAMPLE_RATE_8KHZ);
              PM_VS_PCM_Codec_Config(&PCMCodecConfig8k);
              printf(" WBS is disabled \n\r");
              printf("\r\n Set CC256xB PCM with 8Khz sample rate \r\n");
            }
#endif

            break;
         case hetHFRIncomingCallStateIndication:
            BD_ADDRToStr(EventData->EventData.IncomingCallStateIndicationEventData.RemoteDeviceAddress, BoardStr);
            printf("hetHFRIncomingCallStateIndication, BD_ADDR: %s, CallState = %d \r\n",
                   BoardStr, EventData->EventData.IncomingCallStateIndicationEventData.CallState);
            break;
         default:
...
static int HFP_Init(char *devname, unsigned long flags)
{
	...
	{
	   ParamList.NumberofParameters = 1;
	   ParamList.Params[0].intParam = 0x040424;
	...
	if(ret_val == 0)
	{
	   ParamList.NumberofParameters = 1;
	   ParamList.Params[0].intParam = 1;
	   ret_val = HFRRegisterEventCallback(&ParamList);
	}
}
	   ...
	   for(i=1; i<argc; i++)
	   {
		   ...
		   if(argv[i][0] == '-')
		   {
			   ...
			   case 'h':
				   ...
				   printf("        Options: -i : Trigger HFP initialization\n");
...
int main(int argc, char *argv[])
{
   ...
   {
	   ret_val = HFP_Init(localdevname, flags);
   }
