...
#define HFRM_ARBITRARY_COMMAND_INDICATION_EVENT_DATA_SIZE      (sizeof(HFRM_Arbitrary_Command_Indication_Event_Data_t))
   ...
   /* The following event is dispatched to a local Audio Gateway service*/
   /* when the remote Hands Free device sends its list of supported     */
   /* codecs.  The RemoteDeviceAddress member identifies the local      */
   /* connection receiving this indication.  The NumSupportedCodecs     */
   /* member specifies the number of codecs and the AvailableCodecList  */
   /* member is a pointer to the list of codecs.                        */
...
#define HFRM_AVAILABLE_CODEC_LIST_INDICATION_EVENT_DATA_SIZE      (sizeof(HFRM_Available_Codec_List_Indication_Event_Data_t))
   ...
   /* The following event is dispatched to a local Hands Free service   */
   /* when the remote Audio Gateway device sends its list of requests to*/
   /* select a codec.  The RemoteDeviceAddress member identifies the    */
   /* local connection receiving this indication.                       */
...
#define HFRM_CODEC_SELECT_INDICATION_EVENT_DATA_SIZE      (sizeof(HFRM_Codec_Select_Indication_Event_Data_t))
   ...
   /* The following event is dispatched to a local Audio Gateway service*/
   /* when the remote Hands Free device confirms a codec selection      */
   /* request.  The RemoteDeviceAddress member identifies the local     */
   /* connection receiving this indication.  The AcceptedCodec member   */
   /* will be true if the remote device accepted the codec and false if */
   /* it did not.                                                       */
...
#define HFRM_CODEC_SELECT_CONFIRMATION_EVENT_DATA_SIZE      (sizeof(HFRM_Codec_Select_Confirmation_Event_Data_t))
   ...
   /* The following event is dispatched to a local Audio Gateway service*/
   /* when the remote Hands Free device requests that the Audio Gateway */
   /* initiate a codec select request in order to start the audio       */
   /* connection setup procedure.  The RemoteDeviceAddress member       */
   /* identifies the local connection receiving this indication.        */
...
BTPSAPI_DECLARATION int BTPSAPI HFRM_Send_Arbitrary_Response(unsigned int HandsFreeManagerEventCallbackID, BD_ADDR_t RemoteDeviceAddress, char *ArbitraryResponse);
   ...
   /* The following function is responsible for sending the available   */
   /* codecs to the remote Audio Gateway device.  This function may only*/
   /* be performed by a Hands Free device with a valid service level    */
   /* connection.  The final two parameters to this function are the    */
   /* number of Codec IDs that are being specified and a pointer to an  */
   /* array of the Codec ID that is being supported.  This function     */
   /* returns zero if successful or a negative return error code if     */
   /* there was an error.                                               */
   /* * NOTE * The HandsFreeManagerEventCallbackID parameter *MUST* be  */
   ...
   /*          service type). There can only be a single control        */
   /*          callback registered for each service type in the system. */
BTPSAPI_DECLARATION int BTPSAPI HFRM_Send_Available_Codecs(unsigned int HandsFreeManagerEventCallbackID, BD_ADDR_t RemoteDeviceAddress, unsigned int NumberAvailableCodecs, unsigned char *CodecIDList);

#ifdef INCLUDE_BLUETOOTH_API_PROTOTYPES
   typedef int (BTPSAPI *PFN_HFRM_Send_Available_Codecs_t)(unsigned int HandsFreeManagerEventCallbackID, BD_ADDR_t RemoteDeviceAddress, unsigned int NumberAvailableCodecs, unsigned char *CodecIDList);
#endif
   ...
   /* function accepts as its input parameters the connection type      */
   /* indicating which connection will process the command and the codec*/
   /* ID of the codec that is being selected.  This function returns    */
   /* zero if successful or a negative return error code if there was an*/
   /* error.                                                            */
   /* * NOTE * The HandsFreeManagerEventCallbackID parameter *MUST* be  */
   /*          the callback ID that was registered via a successful call*/
