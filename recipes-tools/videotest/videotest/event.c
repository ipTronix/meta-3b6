...
psEvHnd evOpen(char* name)
{
  ...
  pHnd = (psEvHnd)malloc(sizeof(sEvHnd));
  if(!pHnd){
    return NULL;
  }
  ...
  ret = ioctl(pHnd->fd, EVIOCGVERSION, &pHnd->version);
  if(ret){
  ...
  ioctl(pHnd->fd, EVIOCGID, pHnd->id);
...
/*
  if (iev.type == EV_SYN) {
    printf("Event: time %ld.%06ld, -------------- %s ------------\n",
      iev.time.tv_sec, iev.time.tv_usec, iev.code ? "Config Sync" : "Report Sync" );
  } else if (iev.type == EV_MSC && (iev.code == MSC_RAW || iev.code == MSC_SCAN)) {
    printf("Event: time %ld.%06ld, type %d (%s), code %d (%s), value %02x\n",
      iev.time.tv_sec, iev.time.tv_usec, iev.type,
      events[iev.type] ? events[iev.type] : "?",
      iev.code,
      names[iev.type] ? (names[iev.type][iev.code] ? names[iev.type][iev.code] : "?") : "?",
      iev.value);
  } else {
...
*/
  eve.time.tv_sec  = iev.time.tv_sec ;
  eve.time.tv_usec = iev.time.tv_usec;
  eve.type  = iev.type ;
  eve.code  = iev.code ;
  eve.value = iev.value;
