...
//  struct mxcfb_gbl_alpha    ga;
  ...
  ret = ioctl(pHnd->fd, FBIOGET_VSCREENINFO, &pHnd->var);
  if(ret<0){
    DBG_ERROR("ioctl FBIOGET_VSCREENINFO %d %s\n", errno, strerror(errno));
    return -1;
  }
  if(nb<1){
    nb = 1;
...
*/
  ...
  if(ret<0){
    //ret =
    ioctl(pHnd->fd, FBIOGET_VSCREENINFO, &pHnd->var);
    DBG_ERROR("ioctl FBIOPUT_VSCREENINFO %d %s\n", errno, strerror(errno));
    return -1;
  }
/*
  // Put global alpha
